"""Detailed color review pages, separate from the public preview."""
from color import (oklch_to_hex, hex_to_oklch, clamp_chroma, deltaE_ok,
                   wcag, apca_lc, cvd_hex, hex_to_rgb, rgb_to_hex)
from palette import (STEPS, ACCENTS, L_BASE, BG, PAPER_BG, ROLES,
                     BASE, PAPER_BASE, WHITE, BG2_OFFSET, BLACK_L, BLACK_C, resolve_ref)
...
def html_swatches(pal):
    ...
    h.append('<h1>Color swatches</h1>'
             '<p class="small">Generated from scripts/palette.py. '
             'See DESIGN.md for the color model and evaluation targets.</p>')

    # Compare the current background in several contexts.
    h.append('<h2>1. Light background: '
    ...
    for k, d in (('bg', pal['bg']),):
        h.append(
            ...
            f'</div>')
    # Compare the Lucretia background with Flexoki paper under identical text.
    ...
    # Neutral base scale.
    h.append('<h2>2. Neutral base scale</h2><div>')
    ...
    h.append('<h2>4. Sparse syntax highlighting</h2>'
...
def rate(w, lc):
    """Classify contrast using the WCAG/APCA targets from DESIGN.md."""
    a = abs(lc)
    if w >= 7 and a >= 90:
        return 'p2', 'High body'
    if w >= 4.5 and a >= 75:
...
def html_contrast(pal):
    ...
    h = [f'<!doctype html><meta charset="utf-8"><title>contrast</title><style>{CSS}</style>',
         ...
         '<p class="small">Ratings follow the targets in DESIGN.md: '
    ...
    h.append(matrix('Dark theme', dark_fgs, dark_bgs,
                    ...
                    'WCAG and APCA use different contrast models; inspect both for dark text profiles.'))
    ...
    h.append('<h2>Tint Fill Visibility (50-200): ΔEok vs Background</h2>'
             ...
             'The screening threshold is ΔEok ≥ 0.02 (DESIGN.md). '
             'Projection can still erase these fills, so use a border or luminance gap as needed.</p>')
    ...
    h.append('<h2>Tint Steps (50-200) as Text: APCA on Same-Hue Dark Backgrounds (700-950)</h2>'
             ...
             'This documents use rules such as red-100 text only on red-800 or darker (DESIGN.md).</p>')
    for n in ACCENTS:
...
# ============================================================
# cvd.html (DESIGN.md CVD simulation)
# ============================================================
...
def html_cvd(pal):
    ...
    h = [f'<!doctype html><meta charset="utf-8"><title>cvd</title><style>{CSS}</style>',
         ...
         '<p class="small">DESIGN.md: checks whether emphasis colors (red/blue) and '
         ...
         'with no meaning carried by color alone.</p>']

    for step, usage in ((600, 'light emphasis and syntax band'),
                        (400, 'shape/chart band and dark strings')):
...
def theme_ctx(pal, kind):
    ...
    the dark UI roles for these mocks.
    """
...
def slide_mock(pal):
    ...
    return (
        ...
        f'each keep one fixed role. <b style="color:{pr["emphasis-2"]}">Red marks warnings</b>. '
        f'Secondary details use quieter text.</div>'
        f'<div style="background:{pr["note-fill"]};color:{pr["note-text"]};border-radius:6px;'
        f'padding:8px 12px;margin:12px 0;font-size:13px">Warning: The file could not be opened. '
        f'Check its location and try again.</div>'
        f'<div style="background:{pr["info-fill"]};color:{pr["info-text"]};border-radius:6px;'
        f'padding:8px 12px;font-size:13px">Note: The table shows the same values '
        f'as the chart.</div>'
...
def markdown_mock(pal, kind):
    ...
    return (
        ...
        f'border-bottom:1px solid {md["hr"]};padding-bottom:6px">Design Memo '
        ...
        f'padding:1px 5px">--color-accent</code> and other short tokens legible. '
        ...
        f'<mark style="background:{hl["green"]};color:{mark_tx};padding:0 2px">A green highlight</mark>, '
        ...
        f'padding-left:12px;margin:10px 0">Compare text and links '
        f'on the same background.</div>'
...
def editor_mock(pal, kind):
    ...
    # The sample touches every syntax role.
...
def gallery_card(pal, kind):
    ...
    for i, (src, fallback, cap) in enumerate(GALLERY_PHOTOS):
        h.append(
            ...
            f'</div>'
            f'<div style="color:{ga["caption"]};font-size:12px;margin:4px 0 0">Caption (caption)</div>'
...
# ============================================================
# roles.html (DESIGN.md role tables and usage mocks)
# ============================================================

def html_roles(pal):
    h = [f'<!doctype html><meta charset="utf-8"><title>roles</title><style>{CSS}</style>',
         '<h1>Role assignments</h1>',
         '<p class="small">Roles come from scripts/palette.py and are exported to '
         'dist/palette/palette.json. These browser examples are not app screenshots.</p>']
    ...
    h.append('<h2>Mock 3: Code Editor (syntax_light / syntax_dark + dist/vscode equivalent)</h2>'
             '<p class="small">Sparse highlighting: variables and calls stay neutral (Tier 1); '
    ...
    h.append('<h2>Mock 4: Gallery (photo-surface + scrim)</h2>'
             ...
             'bright, dark, and colorful image areas (DESIGN.md).</p>')
...
def html_overview(pal):
    h = [f'<!doctype html><meta charset="utf-8"><title>overview</title><style>{CSS}</style>',
         ...
         'Role definitions: scripts/palette.py. Exported values: dist/palette/palette.json. '
    ...
    h.append('<h2>2. Syntax Highlighting - Code Editor</h2>'
             '<p class="small">Sparse highlighting in all three profiles: variables stay neutral; '
    ...
    h.append('<h2>2b. Syntax Highlighting - Markdown</h2>'
             ...
             'from the dark UI roles (no additional palette tokens).</p>')
    for kind in MOCK_THEMES:
    ...
    h.append('<h2>7. Plot - chart-1..7 in practice</h2>'
             ...
             'assets/plots/). The same seven series on the light bg, the paper bg, and '
             'MATLAB-style axes as a reference.</p>')
    for name, cap in (('lucretia_light', 'Light bg #FDFCF7, grid'),
                      ('lucretia_paper', 'Paper bg #F8F5EB, grid'),
                      ('matlab_default', 'MATLAB-style axes (reference)')):
    ...
    h.append('<h2>8. Gallery - Light / Dark / Paper</h2>'
             ...
             '(DESIGN.md). The paper card is provisional: the reading profile has no '
    ...
    h.append('<h2>9. Slide</h2>'
             '<p class="small">Light slide mock using light_high and presentation roles.</p>')
    h.append(slide_mock(pal))
    return ''.join(h)


# ============================================================
# paper.html (DESIGN.md reading profile review)
# ============================================================
...
def html_paper(pal):
    ...
    h = [f'<!doctype html><meta charset="utf-8"><title>paper</title><style>{CSS}</style>',
         '<h1>Paper reading profile</h1>',
         '<p class="small">Paper is a warm palette for reading, with lower background luminance '
         'than Light. It has no dedicated presentation or gallery roles. '
         'See DESIGN.md for its background and warm neutral scale.</p>']

    # Compare Paper with the Light and Flexoki backgrounds under identical text.
    h.append('<h2>1. Paper background compared with reference colors</h2>')
    long_p = READING_SAMPLE
    o = P['oklch']
    refs = [('paper-bg', P['bg'], f'({o[0]}, {o[1]}, {o[2]}°)'),
            ('light bg', pal['bg']['bg'], '(0.990, 0.006, 95°)'),
            ('Flexoki paper', '#FFFCF0', '(0.990, 0.016, 95°)')]
    for name, bgh, oklch in refs:
        h.append(
            f'<div class="card" style="background:{bgh};width:340px">'
            f'<b style="color:{pa["tx"]}">{name}</b> '
    ...
    h.append(
        ...
        f'<div style="border-left:3px solid {md["quote-border"]};color:{md["quote-text"]};'
        f'padding-left:14px;margin:12px 0">A paragraph gives a better sense of a text color '
        f'than a single word.</div>'
...
# ============================================================
# degrade.html (DESIGN.md degradation simulation)
# ============================================================
DEGRADE_MODES = [
...
def html_degrade(pal):
    ...
    h = [f'<!doctype html><meta charset="utf-8"><title>degrade</title><style>{CSS}</style>',
         '<h1>Degradation Simulation (DESIGN.md)</h1>',
